...
class ALU extends Module {
  ...
  io.out := MuxLookup(mux_alu_opcode, "hDEADF00D".U)(
    Seq(
      ...
      AluOpCode.ShiftRightArithmetic -> (io.in1.asSInt >> io
        .in2(4, 0)).asUInt, // Chisel only performs arithmetic right-shift on SInt
  ...
  when(true.B) {
    printf(
      "io.alu_opcode = %d mux_alu_opcode = %d io.in1 = %x io.in2 = %x io.out = %x\n",
      io.alu_opcode,
      mux_alu_opcode,
      io.in1,
      io.in2,
