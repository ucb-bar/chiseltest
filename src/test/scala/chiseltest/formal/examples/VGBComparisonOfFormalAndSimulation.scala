...
/** Chisel port of the "Verification Gentleman" blog post on formal vs. simulation. src:
  * https://github.com/verification-gentleman-blog/comparison-of-formal-and-simulation blog:
  * https://blog.verificationgentleman.com/2020/11/comparison-of-formal-and-simulation-part-1.html
  */
class VGBComparisonOfFormalAndSimulation
    extends AnyFlatSpec
    with ChiselScalatestTester
    with Formal
    with FormalBackendOption {
...
class ShapeProcessorProps extends Module {
  ...
  assert(
    IfThen(
      past(io.write && isReservedShape(shapeOnWriteBus)),
      stable(sfr)
    )
  )
  assert(
    IfThen(
      past(io.write && isReservedOperation(operationOnWriteBus)),
      stable(sfr)
    )
  )
  ...
  assert(
    IfThen(
      past(
        io.write && shapeOnWriteBus === KeepShape.asUInt &&
  ...
  assert(
    IfThen(
      past(
        io.write && operationOnWriteBus === KeepOperation.asUInt &&
