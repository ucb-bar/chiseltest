...
abstract class StepCompliance(sim: Simulator) extends ComplianceTest(sim) {
  private val base =
    """circuit test:
      |  module test:
      |    input clock: Clock
  ...
  it should "be able to step a simple circuit with a single clock" in {
    val dut = load(base)
  ...
  it should "throw an exception when trying to step a circuit with no clock" in {
    val dut = load(combCircuit)
    assertThrows[RuntimeException] { // TODO: custom exception
      dut.step()
    }
    dut.finish()
  }

  private def counter(clocks: List[String]): String = {
  ...
  it should "support stepping multiple cycles" in {
    val src = counter(List("clock"))
    ...
    dut.step(List("clock"), 20)
    assert(dut.peek("clock_count") == 21)
    dut.step(List("clock"), 1234)
    assert(dut.peek("clock_count") == 1234 + 20 + 1)

    dut.finish()
  }

  it should "support stepping a clock that is not named 'clock'" in {
    val clock = "test123"
    val dut = load(counter(List(clock))) // a counter with a single clock with non standard name
    dut.poke("reset", 1)
    dut.step()
    dut.poke("reset", 0)
...
abstract class StepMultiClockCompliance(sim: Simulator) extends ComplianceTest(sim) {
  // TODO
}
