...
class AsyncResetReg(val instanceName: String) extends ScalaBlackBox with LazyLogging {
  ...
  /** Important note: The dependency of io_d on io_q makes this test work by making sure that the assignments are sorted
    * correctly topologically. They mirror a similar pattern used on registers, necessary for treadle to be able to
    * update the circuit in a single pass.
    * @return
    */
