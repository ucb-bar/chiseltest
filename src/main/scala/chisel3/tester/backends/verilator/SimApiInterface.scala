...
import chisel3.tester.backends.{TestApplicationException, TesterProcess, getPorts}
...
/** Provides an interface layer to an external verilog simulation program through IPC channels
  ...
  */
private[tester] class SimApiInterface(dut: MultiIOModule, cmd: Seq[String]) extends LazyLogging {
  //
  ...
  private object SIM_CMD extends Enumeration { val RESET, STEP, UPDATE, POKE, PEEK, FORCE, GETID, GETCHK, FIN = Value }
  ...
  private[tester] val process = TesterProcess(cmd, _logs)

  // Set up a Future to wait for (and signal) the test process exit.
  import ExecutionContext.Implicits.global
  private[tester] val exitValue = Future(blocking(process.exitValue))
...
private[tester] class Channel(name: String) {
