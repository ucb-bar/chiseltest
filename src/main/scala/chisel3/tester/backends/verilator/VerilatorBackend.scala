...
import chisel3.tester.Utils.unsignedBigIntToSigned
import chisel3.tester.backends.{BackendExecutive, verilogToVerilator}
import chisel3.tester.internal._
import chisel3.tester.{Region, TimeoutException}
import chisel3.{SInt, _}
import firrtl.annotations.ComponentName
import firrtl.stage.{CompilerAnnotation, OutputFileAnnotation}
...
extends BackendInstance[T] with ThreadedBackend[T] {
  ...
  protected def getClock(clk: Clock): Boolean = simApiInterface.peek(dataNames(clk)) match {
...
object VerilatorExecutive extends BackendExecutive {
  ...
  def start[T <: MultiIOModule](dutGen: () => T, annotationSeq: AnnotationSeq): BackendInstance[T] = {
    ...
    try {
      val targetDir = annotationSeq.collectFirst { case TargetDirAnnotation(t) => t }.get
      ...
      val verilogFileName = compiledAnnotations.collectFirst { case OutputFileAnnotation(f) => f }.get + ".v"
      ...
      val moreVerilatorFlags = compiledAnnotations.collectFirst { case VerilatorFlags(f) => f }.getOrElse(Seq.empty)
      val moreVerilatorCFlags = compiledAnnotations.collectFirst { case VerilatorCFlags(f) => f }.getOrElse(Seq.empty)
      val suppressVerilatorVCD = compiledAnnotations.exists{ case SuppressVerilatorVCD => true ; case _ => false }
      val commandEditsFile = compiledAnnotations.collectFirst { case CommandEditsFile(f) => f }.getOrElse("")

      val verilatorFlags = moreVerilatorFlags ++ ( if (suppressVerilatorVCD) Seq() else Seq("--trace") )
      assert(
        verilogToVerilator(
          circuit.name,
          new File(targetDir),
      ...
      assert(chisel3.Driver.cppToExe(circuit.name, targetDirFile).! == 0)

      val command = compiledAnnotations
              .collectFirst[Seq[String]] { case TestCommandOverride(f) => f.split(" +") }
...
//          case (p, n) => (p, s"$n")
      ...
      //TODO: Original commented code makes it harder to figure out what went wrong above
      case t: Throwable =>
