...
import chisel3._
import chiseltest.backends.{SimulatorInterface, VPIBackend}
...
class VPIInterface(val annotations: AnnotationSeq)
  ...
  val (inputsNameToChunkSizeMap, outputsNameToChunkSizeMap) = {
    val (inputs, outputs) = getTopPorts(annotations).partition(_.direction == firrtl.ir.Input)
  ...
  private[chiseltest] val process = TesterProcess(getCommand(annotations).get, _logs)
  ...
  private def start(): Unit = {
    println(s"""STARTING ${getCommand(annotations) mkString " "}""")
    mwhile(!recvOutputs) {}
  }
