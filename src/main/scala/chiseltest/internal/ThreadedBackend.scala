...
/** Base trait for backends implementing concurrency by threading.
  ...
  */
class ThreadedBackend(annotations: AnnotationSeq) extends LazyLogging {
  ...
  def timestep(): Unit = {
    ...
    signalPeeks.toSeq.foreach {
      case (peekSignal, peeks) =>
    ...
    val pokeTimescopes = rootTimescope.get.overridingPokes.toMap.map {
      case (signal, _) =>
