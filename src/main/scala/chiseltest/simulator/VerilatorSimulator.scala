...
import chiseltest.simulator.jna.{JNASimulatorContext, JNAUtils, VerilatorCppJNAHarnessGenerator}
import firrtl._
...
/** tells the backend to use an external process instead of loading a library via JNA */
...
private object VerilatorSimulator extends Simulator {
  ...
  override def createContext(state: CircuitState): SimulatorContext = {
    ...
    if (shouldCache) {
      ...
      Caching.cacheSimulationBin(simName, state, createContextFromScratch(_, useJNA = false), recreateCachedContext)
    } else {
  ...
  private def createContextFromScratch(state: CircuitState, useJNA: Boolean): SimulatorContext = {
    ...
    val (cppHarness, jnaClassName) = generateHarness(targetDir, toplevel, waveformExt, useJNA)
    ...
    if (useJNA) {
      ...
      val libPath = targetDir / jnaClassName
  ...
  private def runVerilator(
    ...
    val flagAnnos: AnnotationSeq = if (useJNA) {
      VerilatorLinkFlags(JNAUtils.ldFlags) +: VerilatorCFlags(JNAUtils.ccFlags) +: annos
    } else { annos }
