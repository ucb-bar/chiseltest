...
import chiseltest.simulator.jna.{JNASimulatorContext, JNIUtils, VerilatorCppJNAHarnessGenerator}
import firrtl._
...
private object VerilatorSimulator extends Simulator {
  ...
  override def createContext(state: CircuitState): SimulatorContext = {
    val shouldCache = Caching.shouldCache(state)
    val useJNA = !state.annotations.contains(VerilatorUseExternalProcess)
    if (shouldCache) {
      if (useJNA) {
        println("WARN: caching not supported with JNA interface, switching to old IPC interface")
      }
      val simName = s"${VerilatorSimulator.name} ${VerilatorSimulator.majorVersion}.${VerilatorSimulator.minorVersion}"
      Caching.cacheSimulationBin(simName, state, createContextFromScratch(_, false), recreateCachedContext)
    } else {
      createContextFromScratch(state, useJNA)
    }
  }
  ...
  private def createContextFromScratch(state: CircuitState, useJNA: Boolean): SimulatorContext = {
    // we will create the simulation in the target directory
    val targetDir = Compiler.requireTargetDir(state.annotations)
    val toplevel = TopmoduleInfo(state.circuit)
    ...
    val (cppHarness, jniClassName) = generateHarness(targetDir, toplevel, waveformExt, useJNA)
    ...
    val verilatedDir = runVerilator(toplevel.name, targetDir, cppHarness, state.annotations, useJNA)
  ...
  private def generateHarness(
    ...
    val className = if (useJNA) {
      val (code, className) = VerilatorCppJNAHarnessGenerator.codeGen(
        toplevel,
        vcdFile,
        targetDir,
