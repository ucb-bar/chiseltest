...
/** Provides an interface layer to an external verilog simulation program through IPC channels
  ...
  */
private[chiseltest] class SimApiInterface(circuit: Circuit, cmd: Seq[String])
    extends LazyLogging {
  def getPorts(circuit: Circuit) = circuit.modules.collectFirst { case Module(_, name, ports, _) if name == circuit.main => ports }.get .partition(_.direction == firrtl.ir.Input)
  //
  ...
  val (inputsNameToChunkSizeMap, outputsNameToChunkSizeMap) = {
    val (inputs, outputs) = getPorts(circuit)
