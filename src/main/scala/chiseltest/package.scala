...
package object chiseltest {
  ...
  implicit class testableData[T <: Data](x: T) {
    ...
    def peek(): T = x match {
      ...
      case (x: UInt) => Context().peekBits(x).asUInt(DataMirror.widthOf(x)).asInstanceOf[T]
      case (x: SInt) => Context().peekBits(x).asSInt(DataMirror.widthOf(x)).asInstanceOf[T]
      case (x: FixedPoint) => {
        val multiplier = BigDecimal(2).pow(x.binaryPoint.get)
        (BigDecimal(Context().peekBits(x)) / multiplier).F(x.binaryPoint).asInstanceOf[T]
      }
      case x: Interval =>
        Context().peekBits(x).I(x.binaryPoint).asInstanceOf[T]
      case (x: Record) => {
    ...
    protected def expect(value: T, message: Option[String]): Unit = (x, value) match {
      case (x: Bool, value: Bool) =>
        Context().expectBits(x, value.litValue, message, Some(BitsDecoders.boolBitsToString))
      // TODO can't happen because of type parameterization
      case (x: Bool, value: Bits) =>
        throw new LiteralTypeException(s"cannot expect non-Bool value $value from Bool IO $x")
      case (x: Bits, value: UInt) => Context().expectBits(x, value.litValue, message, None)
      case (x: SInt, value: SInt) => Context().expectBits(x, value.litValue, message, None)
      // TODO can't happen because of type parameterization
      case (x: Bits, value: SInt) =>
        throw new LiteralTypeException(s"cannot expect non-SInt value $value from SInt IO $x")
      case (x: FixedPoint, value: FixedPoint) => {
        require(x.binaryPoint == value.binaryPoint, "binary point mismatch")
        Context().expectBits(x, value.litValue, message, Some(BitsDecoders.fixedToString(x.binaryPoint)))
      }
      case (x: Interval, value: Interval) =>
        require(x.binaryPoint == value.binaryPoint, "binary point mismatch")
        Context().expectBits(x, value.litValue, message, Some(BitsDecoders.fixedToString(x.binaryPoint)))
      ...
      case (x: EnumType, value: EnumType) => {
        require(DataMirror.checkTypeEquivalence(x, value), s"EnumType mismatch")
        Context().expectBits(x, value.litValue, message, Some(BitsDecoders.enumToString(x)))
      }
